...
Definition Object :ClassName := 0.

Definition pizza_interface_name:InterfaceName := 0.
Definition crust_class_name:ClassName := 1. (*Object is 0, see Line 19*)
Definition price:MethodName := 0.

Definition int_0_1 := (TRefn TInt (PCons (ExpBinOp Or (ExpBinOp Eq (BV 0) (Ic 1)) (ExpBinOp Eq (BV 0) (Ic 2))) PEmpty) ).
Definition unit_type := (TRefn TBool PEmpty ).

Definition price_mdecl := MDecl int_0_1 price PEmpty unit_type.
Definition mdecls := price_mdecl::nil.
Lemma mdecl_nodup: NoDup (refs mdecls).
Proof.
  apply NoDup_cons. 
  auto.
  apply NoDup_nil.
Qed.
Definition pizza_interface := RefIDecl pizza_interface_name mdecls mdecl_nodup.
Definition RefIT := pizza_interface::nil.


Definition price_crust_mdecl := MDecl int_0_1 price PEmpty unit_type.
Definition price_crust_mdefn := MDefn price_crust_mdecl (Ic 1).
Definition crust_mdefns := price_crust_mdefn::nil.
Lemma mdefn_nodup: NoDup (refs crust_mdefns).
Proof.
  apply NoDup_cons. 
  auto.
  apply NoDup_nil.
Qed.
Definition crust_fdefns:[RefFieldDecl] := nil.
Lemma fdefn_nodup : NoDup (refs crust_fdefns).
Proof.
  auto.
Qed.
Definition crust_ctor := KDecl crust_class_name crust_fdefns nil nil.
Definition crust_class := RefCDecl crust_class_name Object (pizza_interface_name::nil) crust_fdefns fdefn_nodup crust_ctor crust_mdefns mdefn_nodup.
Definition RefCT := crust_class::nil.